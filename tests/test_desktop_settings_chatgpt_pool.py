...
class DesktopSettingsChatgptPoolTest(unittest.TestCase):
    ...
    def test_settings_js_renders_account_pool_management(self):
        ...
        self.assertIn("data-chatgpt-pool-oauth-feedback", source)
        self.assertIn("chatgptPoolOAuthSessionId", source)
        self.assertIn("登录完成后，复制浏览器地址栏的完整 URL 并粘贴到这里", source)
        self.assertIn("请先粘贴授权完成后的回调 URL 或 Code", source)
        self.assertIn("loadChatgptPoolAccounts({ preserveScroll: true })", source)
